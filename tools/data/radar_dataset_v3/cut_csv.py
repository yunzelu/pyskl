            ...
            if os.path.exists(csv_file):
                ...
                for i, interval in enumerate(intervals):
                    if interval['label'].lower() == target_label_lower:
                        ...
                        # --- NEW LOGIC: Target is already too big! ---
                        if t_len >= max_len:
                            # We cannot add background. Just take the middle of the target.
                            mid = t_start + (t_len // 2)
                            clip_start = mid - (max_len // 2)
                            clip_end = clip_start + (max_len - 1)
                            
                        # --- Target is small enough, follow normal logic ---
                        elif add_bg:
                            ...
                            # Stationary Logic (when target is shorter than max_len)
                            if t_len < min_len:
