...
ann_file = 'data/radar/pkl/loso_yunze.pkl'
train_pipeline = [
    ...
    dict(type='RobustMasking', threshold=0.5, mask_prob=0.2),
...
val_pipeline = [
    ...
    dict(type='PoseDecode'),
    dict(type='FormatGCNInput', num_person=1),
    dict(type='RobustMasking', threshold=0.5, mask_prob=0.0),
    dict(type='DropScoreChannel'),
    dict(type='Collect', keys=['keypoint', 'label'], meta_keys=[]),
    dict(type='ToTensor', keys=['keypoint'])
]
...
# runtime settings
log_level = 'INFO'
work_dir = './work_dirs/stgcn++/radar_v3/loso/yunze'
